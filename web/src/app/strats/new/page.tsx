...
// ---------------------------------------------------------------------------
// Inner editor (needs useSearchParams, must be inside Suspense)
// ---------------------------------------------------------------------------
...
function StrategyEditorInner() {
  ...
  const mapId = normalizeStrategyMapId(searchParams.get('mapId'))
  ...
  const { present: elements, push: pushElements, undo, redo, canUndo, canRedo } = useHistory([])

  // ---------------------------------------------------------------------------
  // Tool change — open brawler picker when brawler tool selected
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // Place brawler at center of visible canvas area
  // ---------------------------------------------------------------------------

  const handlePlaceBrawler = (brawler: Brawler, team: 'blue' | 'red') => {
    // Center of the reference canvas (1080×1920)
    const cx = 1080 / 2
    const cy = 1920 / 2
  ...
  // ---------------------------------------------------------------------------
  // Keyboard: undo / redo (skip when typing in title)
  // ---------------------------------------------------------------------------
      ...
      <header className="flex md:hidden flex-col shrink-0 bg-[#161B22] border-b border-gray-700">
        {mobileHeaderExpanded ? (
          <div className="relative px-3 pt-2 pb-3">
            ...
            {gameMode ? (
              <p className="mt-2 text-xs text-gray-500 truncate">{gameMode}</p>
            ) : null}
          </div>
        ) : (
